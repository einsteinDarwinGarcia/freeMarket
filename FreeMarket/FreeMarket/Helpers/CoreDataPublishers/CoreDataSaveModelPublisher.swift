...
typealias ActionCoreData = (()->())

struct CoreDataSaveModelPublisher: Publisher {
    typealias Output = Bool
    typealias Failure = NSError
    
    private let action: ActionCoreData
    private let context: NSManagedObjectContext
    
    init(action: @escaping ActionCoreData, context: NSManagedObjectContext) {
        self.action = action
...
extension CoreDataSaveModelPublisher {
    class Subscription<S> where S : Subscriber, Failure == S.Failure, Output == S.Input {
        ...
        init(subscriber: S, context: NSManagedObjectContext, action: @escaping ActionCoreData) {
